...
const SearchBar = (props) => {
  ...
  function handleSumbit(event) {
    ...
    if (searchInput === "" && dropDownFilter === "") {
      alert(
        "Use the dropdown and/or input box to create some search peramaters."
      );
    } else if (searchInput === "" && dropDownFilter !== "") {
      sortByCriteria();
    } else if (searchInput !== "" && dropDownFilter !== "") {
      filterSongs();
    } else {
  ...
  function filterByIncludes() {
    let filteredByIncludes = props.songs.filter((song) => {
      ...
      return noNumsArr.some((value) =>
        value
          .toString()
          .toLowerCase()
          .includes(searchInput.toString().toLocaleLowerCase().trim())
      );
    });
  ...
  function sortByLikes() {
    switch (likesDropDown) {
      case "Most Likes":
        let mostLiked = props.songs
          .slice(0)
          .sort((a, b) => b[dropDownFilter] - a[dropDownFilter]);
        return mostLiked;
      case "=":
        let likeSearch = props.songs.filter(
          (song) => song.likes === parseInt(searchInput.trim())
        );
        return likeSearch;
      case ">":
        let greaterSearch = props.songs.filter(
          (song) => song.likes >= parseInt(searchInput.trim())
        );
        return greaterSearch;
      case "<":
        let lessThanSearch = props.songs.filter(
          (song) => song.likes <= parseInt(searchInput.trim())
        );
  ...
  return (
    <div className="searchBar">
      <form onSubmit={handleSumbit}>
        ...
        <select
          name="likes comparisons"
          value={likesDropDown}
          hidden={likesBool}
          onChange={(event) => setLikesDropDown(event.target.value)}
        >
        ...
        <input
          ...
          type={searchType}
          value={searchInput}
          placeholder=" More Search Criteria..."
          onChange={(event) => setSearchInput(event.target.value)}
        ></input>
        <button className="btn btn-outline-light" type="submit">
          Search
        </button>
        <button className="btn btn-outline-light" onClick={() => newSearch()}>
          New Search
        </button>
      </form>
    </div>
