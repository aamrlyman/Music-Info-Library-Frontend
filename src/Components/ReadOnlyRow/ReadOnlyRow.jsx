...
useEffect(()=> {},[])
    return (
        <tr className='dataRows' onMouseEnter={()=>setIsHovered(true)} onMouseLeave={()=>setIsHovered(false)} key={props.song.id}>
        {/* <button type="button" class="btn btn-secondary"  data-toggle="tooltip" data-placement="top" title="Tooltip on top">
  Tooltip on top
</button> */}
{ !isHovered ? <td className="likes">
          ...
          </td>: <td><button title={`${props.song.likes} likes`}
            className="btn btn-outline-light"  
            onClick={(event) => likeSong(event, props.song.id)}>
              <i className="fa-solid fa-thumbs-up" >
